...
exports.getSchool = asyncHandler(async (req, res, next) => {
  var school = await School.findById(req.params.id).populate('students')
...
// @desc    Get a single School by tag
// @route   GET /api/v1/schools/tag/:id
// @access  Public
exports.getSchoolByTag = asyncHandler(async (req, res, next) => {
...
exports.uploadStudents = asyncHandler(async (req, res, next) => {
  ...
  customHeader = customHeader.filter((header) => !header.includes('fee'))
  ...
  await Promise.all(
    result.map(async (student) => {
      ...
      var fee = []
      var schoolPayment = undefined
      for (const [key, value] of Object.entries(student)) {
        if (key.includes('fee') && value === 'yes') {
          fee.push({
            name: key.split('_')[1],
            value: parseInt(key.split('_')[2]),
          })
          var { [key]: removedProperty, ...studentDouble } = studentDouble
        }
      }
